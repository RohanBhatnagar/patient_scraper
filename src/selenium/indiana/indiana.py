...
def simulate(zip):
    ...
    driver.get(
        "https://provider.indianamedicaid.com/ihcp/ProviderServices/ProviderSearch.aspx#results")

    driver.implicitly_wait(10)

    ## _____ optometrist

    # radioButton = driver.find_element(by=By.ID, value="radOther")
    # radioButton.click()

    # opt = driver.find_element(by=By.NAME, value="ddlProviderTypes")
    # dropdown = Select(opt)
    # dropdown.select_by_value("18")

    # zip_txt = driver.find_element(by=By.ID, value="txtZip")
    # zip_txt.send_keys(zip)

    # submit = driver.find_element(by=By.NAME, value="cmdSearch")
    # submit.click()

    # --------

    specialties = driver.find_element(by=By.NAME, value='lstSpecialties')
    sDrop = Select(specialties)
    sDrop.deselect_all()
    sDrop.select_by_value('330')
    ...
    src_arr.append(src)
    if "Page: 1 of " in src:
        start = src.find("Page: 1 of ")
        end = src[start:].find("</span>")
        np = src[start+11: start+end]
        np = int(np)
    else:
        np = 1
    print("--> np: " + str(np))
    ...
    if np > 1:
        for i in range(1, np): 
            next = driver.find_element(by=By.NAME, value="cmdNext2")
            next.click()
            print("--> next")
            time.sleep(1)
            src = driver.page_source
...
def main():
    for i in range(0, len(zips), 1):
        print(zips[i])
        source = simulate(zips[i])
        for s in source:
            soup = BeautifulSoup(s, 'html.parser')
            pret = soup.prettify()
            csv_arr = write(pret)
            with open('indiana_opths.csv', 'a') as file:
